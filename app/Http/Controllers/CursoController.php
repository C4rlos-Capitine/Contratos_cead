...
class CursoController extends Controller
{
    ...
    public function ver_detalhes(Request $request)
    {
        ...
        $docente = Docente::select("nome_docente", "apelido_docente")
            ...
            ->firstOr(function () {
                return (object)[
                    'nome_docente' => 'não definido',
                    'apelido_docente' => 'não definido',
                ];
            });

        $docentes = Docente::select("*")
            ->where("id_faculdade_in_docente", $cursos->id_faculdade)
            ->get();

        return view('curso.detalhes', [
            'cursos' => $cursos,
            'docente' => $docente,
            'docentes' => $docentes,
        ]);
    ...
    public function update(Request $cursos)
    {
        try {
