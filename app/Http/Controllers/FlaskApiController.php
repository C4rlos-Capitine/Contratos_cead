...
class FlaskApiController extends Controller
{
    ...
    if($n_areas < 1) {
        // Nova consulta usando get() para retornar uma coleção
        $codAreas = DB::table('area_docente')
            ->select('id_area')
            ->distinct()
    ...
    // Aqui você pode continuar com a lógica, utilizando $codAreas e $n_areas
        ...
        if($existe == true){
            ...
            $areas = [];
            foreach ($codAreas as $area) {
                $areas[] = ['cod_area_in_leciona' => $area->id_area];
            }
        }
        try {
